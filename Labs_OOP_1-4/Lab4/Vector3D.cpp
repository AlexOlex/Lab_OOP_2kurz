...
//Первантажені оператори для роботи з Vector3D:

bool Vector3D::operator<(const Vector3D& secondVec) const
{
    //порівняння довжини поточного вектора |this| < другого вектора |secondVec|
...
Vector3D Vector3D::operator+(const Vector3D& secondVec) const
{
    ...
    Vector3D res;
    res.Init(x + secondVec.x, y + secondVec.y, z + secondVec.z);  //Init для коректної ініціалізації
    return res;
}
...
//перевантажений оператор рівності
bool Vector3D::operator==(const Vector3D& secondVec) const
{
...
//Функція перевірки на рівність використовує перевантажений оператор ==
